...
def build_rushton_impeller(p: STRParams, z_m: float) -> "cadquery.Workplane":
...
def build_impellers(p: STRParams) -> list["cadquery.Workplane"]:
    from str_cad.geometry.registry import get_impeller

    build_impeller = get_impeller(p.impellers.type)
    return [
        build_impeller(p, z_m)
