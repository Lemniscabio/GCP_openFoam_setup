...
def _seed_valid(s):
    ...
    s.upload_bytes("cases/case_0001/case/command.sh", b"mpirun -np ${MPI_RANKS} foamRun -parallel")
...
def test_command_without_mpi_ranks_warns():
    s = InMemoryStorage(); _seed_valid(s)
    s.upload_bytes("cases/case_0001/case/command.sh", b"mpirun -np 8 foamRun -parallel")
    result = validate_case(s, "case_0001")
